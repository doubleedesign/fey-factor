...
export class DbWorks {
	...
	async getPeopleForTvShow(id: number): Promise<Person[]> {
		try {
			const response = await this.pgClient.query({
				text: `
                    WITH person_connections AS (SELECT person_id,
                    ...
                    SELECT people.id,
                           ...
                           people.degree,
                           SUM(CASE
                                   WHEN connections.work_id = $1 THEN connections.episode_count
                                   ELSE 0 END)                                                              AS aggregate_episode_count,
                           COUNT(DISTINCT CASE
                                              WHEN connections.work_id != $1
                                                  THEN connections.work_id END)                             AS other_works_count
                        FROM
                            people
                                JOIN connections
                                ON people.id = connections.person_id
                                JOIN person_connections pc
                                ON people.id = pc.person_id
                        WHERE
                        ...
                        HAVING
                          ...
                          OR
                            ...
                            (people.degree > 1 AND
                             COUNT(DISTINCT CASE WHEN connections.work_id != $1 THEN connections.work_id END) > 0)
                        ORDER BY
                            degree ASC,
	...
	async getPersonsRolesForWork(personId: number, workId: number, workType: 'T' | 'F'): Promise<Role[]> {
		try {
			const response = await this.pgClient.query({
				text: `SELECT role_id, roles.name, episode_count
                           FROM
                               connections
                                   JOIN roles
                                   ON connections.role_id = roles.id
                           WHERE
                                 person_id = $1
                             AND work_id = $2
                `,
	...
	async getRankedListOfTvShows(limit: number): Promise<TvShow[]> {
		try {
			const response = await this.pgClient.query({
				text: `
                    WITH filtered_connections AS (SELECT c.person_id,
                                                         c.work_id,
                                                         c.episode_count
                                                      FROM
                                                          connections c
                                                      ...
                                                      WHERE
                                                        ...
                                                        OR (p.degree > 1 AND (SELECT COUNT(DISTINCT work_id)
                                                                                  FROM connections
                                                                                  WHERE person_id = c.person_id) > 1)),
                         --- end of filtered_connections definition
                         aggregated_data      AS (SELECT tv_shows.id,
                                                      ...
                                                      FROM
                                                          tv_shows
                                                              INNER JOIN filtered_connections fc
                                                              ON tv_shows.id = fc.work_id
                                                              INNER JOIN people
                                                              ON fc.person_id = people.id
                                                              INNER JOIN works
                                                              ON tv_shows.id = works.id
                                                      WHERE
                    ...
                    SELECT ad.id,
                           ...
                           ROUND((ad.aggregate_episode_count / ad.episode_count) * ad.inverted_average_degree,
                                 2)::DECIMAL AS weighted_score
                        FROM
                            aggregated_data ad
