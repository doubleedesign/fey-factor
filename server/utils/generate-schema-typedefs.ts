...
export async function generateSchemaTypedefs(file: string) {
	...
	// Get the content of the TypeScript types file generated by pg-to-ts
	...
	// Additional types created for the GraphQL schema that are not entities in the database
	...
	// Some notes about processing of types (TODO: Copy these notes to some centralised docs)
	// The TypeScript AST (Abstract Syntax Tree) is a tree representation of the TypeScript code that is used by these functions
	// You can check the type of a Node using node.kind which returns a number
	// You can find what these numbers mean in/node_modules/typescript/lib/typescript.d.ts - look for enum SyntaxKind
	// Alternatively, paste your types file into https://ts-ast-viewer.com - this provides a UI to navigate the tree and find all the info you might need about the nodes

	try {
		const nonExportedTypes = await collectNonExportedTypes(extraTypesFile); // these contain extra fields to be added to the relevant entities
		await collectExportedTypes(sourceFile);
		await collectExportedTypes(extraTypesFile);
		detectSubtypes();
		await addForeignKeyFields();
		addExtraFields(nonExportedTypes);
		convertAndSaveTypes();
...
/**
 * Process non-exported types (intended to add fields to other types) and return them for use in the functions that will process the exported types
 * Note: These types should start with the same name as the type they are intended to add fields to
 * @param node
 */
async function collectNonExportedTypes(node: ts.Node): Promise<ts.TypeAliasDeclaration[]> {
	const types: ts.TypeAliasDeclaration[] = [];

	if(node.kind === ts.SyntaxKind.SourceFile) {
		ts.forEachChild(node, childNode => {
			if (ts.isTypeAliasDeclaration(childNode)) {
				const isExported = childNode.modifiers?.some(modifier => modifier.kind === ts.SyntaxKind.ExportKeyword);
				if (!isExported) {
					types.push(childNode);
				}
			}
		});
	}

	//console.log(types.map(type => type.name.text));
...
/**
 * Add extra fields to the GraphQL schema that are not in the database tables
 * and are specified using non-exported custom types
 * @param extraFieldTypes
 */
function addExtraFields(extraFieldTypes: ts.TypeAliasDeclaration[]) {
	Object.entries(typeObjects).forEach(([typeName, data]) => {
